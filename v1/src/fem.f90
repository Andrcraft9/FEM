...
!		print *, i, j, x, y, base_f
	...
	real*8 function sp_cell_gbf_gbf(i1, j1, i2, j2, i_cell, j_cell, b1x, b1y, b2x, b2y)
		implicit none
...
!		print *, S1, S2, S3, S4
	...
	! L2 scalar product in cell: ( (vec(b1), grad of base function1), (vec(b2), grad of base function2) )
	real*8 function sp_gbf_gbf(i1, j1, i2, j2, b1x, b1y, b2x, b2y)
		implicit none
		...
		S1 = sp_cell_gbf_gbf(i1, j1, i2, j2, i1, j1, b1x, b1y, b2x, b2y)		
		S2 = sp_cell_gbf_gbf(i1, j1, i2, j2, i1, j1 - 1, b1x, b1y, b2x, b2y)		
		S3 = sp_cell_gbf_gbf(i1, j1, i2, j2, i1 - 1, j1 - 1, b1x, b1y, b2x, b2y)
		S4 = sp_cell_gbf_gbf(i1, j1, i2, j2, i1 - 1, j1, b1x, b1y, b2x, b2y)
...
!					do k = 1, fem_N-1
				do l = max(j - 1, 1), min(j + 1, fem_N - 1)
					do k = max(i - 1, 1), min(i + 1, fem_N - 1)
						...
						S = S + 0.5*sp_gbf_gbf(k, l, i, j, fem_dx, fem_dy, 1.0d0, 1.0d0) 												
				...
				do l = j - 1, j + 1
					do k = i - 1, i + 1
						...
						if ( (l == 0 .or. l == fem_N) .or. (k == 0 .or. k == fem_N) ) then
							...
							S1 = S1 - 0.5d0*sp_gbf_gbf(k, l, i, j, fem_dx, fem_dy, 1.0d0, 1.0d0)
							S1 = g_D(meshx(k), meshy(l)) * S1
							
							S2 = (1.0d0/fem_dt) * sp_bf_bf(k, l, i, j)
							S2 = S2 - 0.5d0*sp_gbf_bf(k, l, i, j, fem_bx, fem_by)
							S2 = S2 - 0.5d0*sp_gbf_gbf(k, l, i, j, fem_dx, fem_dy, 1.0d0, 1.0d0)
							...
							S = S - 0.5d0*sp_gbf_gbf(k, l, i, j, fem_dx, fem_dy, 1.0d0, 1.0d0)
							S = fem_C0(indx_in) * S
						endif
