...
module fem
    ...
    real*8, allocatable :: fem_sp_vgbf_vgbf(:, :)

    ! SUPG parameters
    real*8, private :: fem_supg_d
    real*8, private :: fem_Peh
    ...
    subroutine allocate_fem()
        ...
        allocate(fem_sp_vgbf_vgbf((fem_N - 1)*(fem_N - 1), 9))
...
!			S = S + T*quad_w2*( grad_base_f(i1, j1, x, y, b1x, b1y) * grad_base_f(i2, j2, x, y, b2x, b2y) )
    ...
    ! L2 scalar product in cell: ( (vec(b1), grad of base function1), (vec(b2), grad of base function2) )
    real*8 function sp_cell_vgbf_vgbf(i1, j1, i2, j2, i_cell, j_cell, b1x, b1y, b2x, b2y)
        implicit none
        
        integer*4 :: i1, j1, i2, j2
        integer*4 :: i_cell, j_cell
        real*8 :: b1x, b1y, b2x, b2y
    ...
    ! L2 scalar product in cell: ( (vec(b1), grad of base function1), (vec(b2), grad of base function2) )
    real*8 function sp_vgbf_vgbf(i1, j1, i2, j2, b1x, b1y, b2x, b2y)
        implicit none
        ...
        S1 = sp_cell_vgbf_vgbf(i1, j1, i2, j2, i1, j1, b1x, b1y, b2x, b2y)		
        S2 = sp_cell_vgbf_vgbf(i1, j1, i2, j2, i1, j1 - 1, b1x, b1y, b2x, b2y)		
        S3 = sp_cell_vgbf_vgbf(i1, j1, i2, j2, i1 - 1, j1 - 1, b1x, b1y, b2x, b2y)
        S4 = sp_cell_vgbf_vgbf(i1, j1, i2, j2, i1 - 1, j1, b1x, b1y, b2x, b2y)
        
        sp_vgbf_vgbf = S1 + S2 + S3 + S4		
            ...
            do i = 1, fem_N-1		  
                ...
                do l = j - 1, j + 1
                    do k = i - 1, i + 1
                        ...
                        fem_sp_vgbf_vgbf(indx, sp_indx) = sp_vgbf_vgbf(k, l, i, j, fem_bx, fem_by, fem_bx, fem_by)
                    enddo
                enddo
            enddo
        enddo
            ...
            do i = 1, fem_N-1		
                ...
                do l = max(j - 1, 1), min(j + 1, fem_N - 1)
                    do k = max(i - 1, 1), min(i + 1, fem_N - 1)
                        ...
                        SUPG = fem_supg_d * (1.0d0/fem_dt) * fem_sp_gbf_bf(indx_row, sp_indx)
                        SUPG = SUPG + fem_supg_d*0.5*fem_sp_vgbf_vgbf(indx_row, sp_indx)
                        
                        if (S .ne. 0.0d0) then
                              indx_col = (l-1)*(fem_N-1) + k
                            fem_JA(fem_IA(indx_row) + num_col) = indx_col
                            fem_A(fem_IA(indx_row) + num_col) = S + SUPG
                            num_col = num_col + 1 
                        endif 
...
!		print *, fem_JA
    ...
    subroutine generate_fem_rhs()
        ...
        integer*4 :: sp_indx
        integer*4 :: k, l
        real*8 :: S, SUPG, S1, S2, SUPG1, SUPG2
                ...
                do l = j - 1, j + 1
                    do k = i - 1, i + 1
                        ...
                        if ( (l == 0 .or. l == fem_N) .or. (k == 0 .or. k == fem_N) ) then
                            ...
                            SUPG1 = -fem_supg_d * (1.0d0/fem_dt) * fem_sp_gbf_bf(indx, sp_indx)
                            SUPG1 = SUPG1 - fem_supg_d*0.5d0*fem_sp_vgbf_vgbf(indx, sp_indx)
                            SUPG1 = g_D(meshx(k), meshy(l)) * SUPG1
                            ...
                            SUPG2 = fem_supg_d * (1.0d0/fem_dt) * fem_sp_gbf_bf(indx, sp_indx)
                            SUPG2 = SUPG2 - fem_supg_d*0.5d0*fem_sp_vgbf_vgbf(indx, sp_indx)
                            SUPG2 = g_D(meshx(k), meshy(l)) * SUPG2
                            ...
                            SUPG = fem_supg_d * (1.0d0/fem_dt) * fem_sp_gbf_bf(indx, sp_indx)
                            SUPG = SUPG - fem_supg_d*0.5d0*fem_sp_vgbf_vgbf(indx, sp_indx)
                            SUPG = fem_C0(indx_in) * SUPG
                        endif
                        fem_RHS(indx) = fem_RHS(indx) + S + SUPG
...
!---------Pekle number-----------------------------------------------------------------------------
        ...
        fem_Peh = (fem_h * sqrt(fem_bx**2 + fem_by**2)) / Db
        print *, 'Pekle number is ', fem_Peh

!---------Init SUPG delta--------------------------------------------------------------------------
        if (fem_Peh > 1.0d0) then
            fem_supg_d = (fem_h / (2.0d0 * sqrt(fem_bx**2 + fem_by**2))) * (1.0d0 - 1.0d0/fem_Peh)
        else
            fem_supg_d = 0
        endif
        print *, 'SUPG delta is ', fem_supg_d
        fem_supg_d = 0.05d0
        print *, 'SUPG delta is ', fem_supg_d
